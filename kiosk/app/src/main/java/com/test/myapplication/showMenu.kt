        ...
        while (true) {
            ...
            try {
                ...
                when (select) {
                    ...
                    7 -> {
                        val todayDate = LocalDateTime.now().format(
                            DateTimeFormatter.ofPattern(
                                "yyyy-MM-dd HH:mm:ss"
                            )
                        )
                        val now = LocalDateTime.now().format(DateTimeFormatter.ofPattern("HHmm"))!!
                            .toInt()
                        val basket = BasketSingletonClass.getInstance()
                        basket.mgcBasketInfo()
    ...
    fun beverageInfo(): MutableList<Menu> {
        ...
        beverage.add(
            Menu(
                "흑당버블밀크티라떼",
                3800,
                "티바론 얼그레이 홍차의 깊은 맛을 살린 밀크티 라떼에 진한 흑당과 흑당 버블의 달콤함을 채운 음료"
            )
        )
